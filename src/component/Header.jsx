...
import { globalContext } from "../myCongtext/MyContext"; // ✅ Ensure correct spelling of folder

const Header = () => {
  // 🛒 Redux cart
  const cart = useSelector((state) => state.cartItems.cart || []); // ✅ Prevent crash if undefined
  const cartCount = cart.reduce((acc, item) => acc + (item.quantity || 0), 0);

  // 📦 Context products
  const { data: products = [] } = useContext(globalContext); // ✅ Default empty array if context missing
  ...
  const filteredProducts = products.filter(
    (p) => p?.title?.toLowerCase().includes(searchText.toLowerCase()) // ✅ Avoid crash if title missing
  );
    ...
    <>
      ...
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark px-3">
        <div className="container-fluid d-flex justify-content-between align-items-center flex-wrap">
          <Link className="navbar-brand navlogo1 text-nowrap text-white" to="/"> {/* ✅ Changed <a> to <Link> */}
            Tech-Shop
          </Link>
          ...
          <div className="d-flex align-items-center icon-group me-md-4 ms-auto">
            ...
            <div className="position-relative d-inline-block text-white pe-md-5 px-2">
              <Link to="/cart" className="linknoness"> {/* ✅ removed space in "/cart " */}
            ...
            <div className="position-relative userspa">
              ...
              {dropdownOpen && (
                ...
                >
                  ...
                  <hr className="text-muted" /> {/* ✅ fixed className */}
